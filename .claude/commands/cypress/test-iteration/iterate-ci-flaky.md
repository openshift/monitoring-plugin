...
parameters:
  ...
  - name: focus
    ...
    required: false
  - name: review-window
    description: "Seconds to wait for user feedback after posting fix as PR comment before pushing (default: 0 = no wait)."
    required: false
---
...
### 3. GitHub PR Comment Review (optional)
...
2. Agent waits `review-window` seconds for a reply from the **PR author only**
3. If the author replies on the PR — agent reads the feedback and adjusts the fix
4. If no reply within the window — agent proceeds autonomously
...
5. **Review window** (before pushing, if `review-window` > 0):

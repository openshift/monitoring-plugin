...
if [ -x "$(command -v podman)" ]; then
    if [ "$(uname -s)" = "Linux" ]; then
        ...
        podman run --pull always --arch=amd64 --rm --network=host --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
    else
    ...
    docker run --pull always --arch=amd64 --rm -p "$CONSOLE_PORT":9000 --env-file <(set | grep BRIDGE) $CONSOLE_IMAGE
fi

...
func TestBulkUpdateAlertRules_TooManyRuleIds(t *testing.T) {
	f := newBUFixture(t)
	ids := make([]string, 101)
	for i := range ids {
		ids[i] = "rule-id"
	...
	if !strings.Contains(w.Body.String(), "ruleIds exceeds maximum of 100") {
		t.Errorf("expected 'ruleIds exceeds maximum of 100', got: %s", w.Body)
	}
}
...
func TestBulkUpdateAlertRules_InvalidRuleId(t *testing.T) {
	...
	if resp.Rules[1].Message == nil || !strings.Contains(*resp.Rules[1].Message, "ruleId is empty") {
		t.Errorf("rule[1]: expected 'ruleId is empty', got %v", resp.Rules[1].Message)
	}
}
...
func TestBulkUpdateAlertRules_RestoreToggle(t *testing.T) {
	...
	// RestoreAlertRule will call AlertRelabelConfigs().Get() and then
...
func TestBulkUpdateAlertRules_ClassificationUserRulesNotAllowed(t *testing.T) {
	user1Id, user2Id, _ := buFixtureIDs()
	...
	if resp.Rules[0].StatusCode != http.StatusMethodNotAllowed || resp.Rules[1].StatusCode != http.StatusMethodNotAllowed {
		t.Errorf("expected both rules 405, got %d / %d", resp.Rules[0].StatusCode, resp.Rules[1].StatusCode)
	}
}

...
const DASHBOARD_PROJECT =
  Cypress.env('COO_NAMESPACE') || 'openshift-cluster-observability-operator';
 ...
 */
describe(
  'COO-LightSpeed: show_timeseries',
  { tags: ['@perses-dashboards', '@coo', '@ols'] },
  () => {
    before(() => {
    ...
    after(() => {
      ...
      cy.adminCLI(
        `oc delete persesdashboard -n ${DASHBOARD_PROJECT} -l ` +
          `app.kubernetes.io/managed-by=perses ` +

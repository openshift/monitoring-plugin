...
const THRESHOLDS = {
  FILTER_APPLY: 3_000,
  FILTER_CLEAR: 400,
  TIME_RANGE_SWITCH: 2_000,
  TABLE_EXPAND_100: 1_000,
  TABLE_EXPAND_500: 8_500,
};
...
const recordBenchmark = (label: string, thresholdMs: number) => {
  cy.window({ log: false }).then((win) => {
    ...
    cy.log(msg);
    cy.task('log', msg);
    expect(elapsedMs, `${label} should complete within ${thresholdMs}ms`).to.be.lessThan(thresholdMs);
  });
};

describe('Performance: Interactive Walkthrough', { tags: ['@incidents', '@performance'] }, () => {

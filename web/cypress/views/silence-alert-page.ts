...
export const silenceAlertPage = {
  ...
  alertLabelsSectionDefault: () => {
    ...
    cy.get(Classes.SilenceAlertHelpTextParagraph).contains(alertText).should('be.visible');
  },

  assertLabelNameLabelValueRegExNegMatcher: (labelName: string, labelValue: string) => {
    cy.log('silenceAlertPage.assertLabelNameLabelValueRegExNegMatcher');
    cy.get('h2').contains('Alert labels').siblings('.row').each(($row, rowIndex) => {
    // cy.byClass('row').each(($row, rowIndex) => { //pf-5 cy.get('.row').each(($row, rowIndex) => {
      cy.wrap($row).find('[aria-label="Label name"]').invoke('val').then((cellText) => {
        if (cellText === labelName){
          ...
          cy.wrap($row).find('label').contains('RegEx').should('be.visible'); 
          cy.wrap($row).find('label').contains('Negative matcher').should('be.visible'); 

...
export const metricsPage = {
  ...
  redirectedToMetricsPageWithoutDatapoints: () => {
    ...
    cy.byTestID(DataTestIDs.MetricDisconnectedCheckbox).should('be.visible');
    cy.get(Classes.MetricGraphNoDatapointsFound).contains(MetricGraphEmptyState.NO_DATAPOINTS_FOUND).should('be.visible');
    cy.byTestID(DataTestIDs.TypeaheadSelectInput).should('be.visible');
    cy.byTestID(DataTestIDs.MetricsPageAddQueryButton).should('be.visible');
    cy.byTestID(DataTestIDs.MetricsPageRunQueriesButton).should('be.visible');
    cy.byTestID(DataTestIDs.MetricsPageExpandCollapseRowButton).eq(0).should('be.visible');
  ...
  shouldBeLoadedWithGraph: () => {
    ...
    cy.byTestID(DataTestIDs.MetricStackedCheckbox).should('be.visible');
    cy.byTestID(DataTestIDs.MetricDisconnectedCheckbox).should('be.visible');
    metricsPage.expandCollapseRowAssertion(true, 0, true);
  },
  ...
  expandCollapseAllQueryAssertion: (expanded: boolean) => {
    ...
    cy.byTestID(DataTestIDs.MetricsPageExpandCollapseRowButton).then(($rowIcons) => {
      ...
      for (let i = 0; i < rowIconsCount; i++) {
        if (expanded) {
          cy.get(Classes.MetricsPageExpandedRowIcon).eq(i).should('be.visible');
          cy.byTestID(DataTestIDs.MetricsPageDisableEnableQuerySwitch).eq(i).should('have.attr', 'checked');
        } else { //collapsed
          cy.get(Classes.MetricsPageCollapsedRowIcon).eq(i).should('be.visible');
   ...
   */
  expandCollapseRow: (expanded: boolean, index: number) => {
    cy.log('metricsPage.expandCollapseRow');
    if (expanded) {
      cy.get(Classes.MetricsPageCollapsedRowIcon).eq(index).should('be.visible').click();
    } else {
      cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).should('be.visible').click();
    }
  },
   ...
   * @param withQuery 
   */
  expandCollapseRowAssertion: (expanded: boolean, index: number, withQuery: boolean) => {
    ...
    if (expanded) {
      if (withQuery) {
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).should('be.visible');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageDisableEnableQuerySwitch + '"]').should('have.attr', 'checked');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageQueryTable + '"]').should('be.visible');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageSelectAllUnselectAllButton + '"]').should('be.visible');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageSeriesButton + '"]').should('have.length.gt', 0);
      } else {
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).should('be.visible');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageDisableEnableQuerySwitch + '"]').should('have.attr', 'checked');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageQueryTable + '"]').should('not.exist');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageSelectAllUnselectAllButton + '"]').should('not.exist');
        cy.get(Classes.MetricsPageExpandedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageSeriesButton + '"]').should('not.exist');
      }
    } else {
      cy.get(Classes.MetricsPageCollapsedRowIcon).eq(index).should('be.visible');
      cy.get(Classes.MetricsPageCollapsedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageDisableEnableQuerySwitch + '"]').should('have.attr', 'checked');
      cy.get(Classes.MetricsPageCollapsedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageQueryTable + '"]').should('not.exist');
      cy.get(Classes.MetricsPageCollapsedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageSelectAllUnselectAllButton + '"]').should('not.exist');
      cy.get(Classes.MetricsPageCollapsedRowIcon).eq(index).find('[data-test="' + DataTestIDs.MetricsPageSeriesButton + '"]').should('not.exist');
    }
  },
  ...
  enterGraphTimespan: (timespan: GraphTimespan) => {
    ...
    cy.byTestID(DataTestIDs.MetricGraphTimespanInput).should('be.visible').clear();
    cy.byTestID(DataTestIDs.MetricGraphTimespanInput).should('be.empty').type(timespan + '{enter}');
    cy.get('[id^="' + IDs.ChartAxis0ChartLabel + '"]').should('be.visible');
  },
  ...
  clickKebabDropdown: (index: number) => {
    ...
    cy.byTestID(DataTestIDs.KebabDropdownButton).eq(index).should('have.attr', 'aria-expanded', 'false').click();
  },

  kebabDropdownAssertionWithoutQuery: () => {
    cy.log('metricsPage.kebabDropdownAssertionWithoutQuery');
    metricsPage.clickKebabDropdown(0);
    cy.byTestID(DataTestIDs.MetricsPageDisableEnableQueryDropdownItem).contains(MetricsPageQueryKebabDropdown.DISABLE_ENABLE_QUERY).should('be.visible');
 ...
 * 2w - 14 labels
 */
  graphAxisAssertion: (graphTimespan: GraphTimespan) => {

...
print_current_config() {
    ...
    echo ""
    echo "  -- CNV --"
    print_var "CYPRESS_SKIP_CNV_INSTALL" "${CYPRESS_SKIP_CNV_INSTALL-}"
    print_var "CYPRESS_CNV_UI_INSTALL" "${CYPRESS_CNV_UI_INSTALL-}"
    print_var "CYPRESS_KONFLUX_CNV_BUNDLE_IMAGE" "${CYPRESS_KONFLUX_CNV_BUNDLE_IMAGE-}"
    print_var "CYPRESS_CUSTOM_CNV_BUNDLE_IMAGE" "${CYPRESS_CUSTOM_CNV_BUNDLE_IMAGE-}"
    print_var "CYPRESS_FBC_STAGE_CNV_IMAGE" "${CYPRESS_FBC_STAGE_CNV_IMAGE-}"
...
main() {
    ...
    local skip_cnv_install="${CYPRESS_SKIP_CNV_INSTALL:-false}"
    local cnv_ui_install="${CYPRESS_CNV_UI_INSTALL:-false}"
    local konflux_cnv_bundle="${CYPRESS_KONFLUX_CNV_BUNDLE_IMAGE-}"
    local custom_cnv_bundle="${CYPRESS_CUSTOM_CNV_BUNDLE_IMAGE-}"
    local fbc_stage_cnv_image="${CYPRESS_FBC_STAGE_CNV_IMAGE-}"
    ...
    if [[ "$skip_all_install" == "false" ]]; then
        ...
        # ── Section 3b: CNV Installation ────────────────────────────────
        section_header "CNV (OpenShift Virtualization)"

        local skip_cnv_ans
        skip_cnv_ans=$(ask_yes_no "Skip OpenShift Virtualization installation?" "$(bool_to_default_yn "$skip_cnv_install")")
        skip_cnv_install="false"
        if [[ "$skip_cnv_ans" == "y" ]]; then skip_cnv_install="true"; fi

        if [[ "$skip_cnv_install" == "false" ]]; then
            local cnv_ui_ans
            cnv_ui_ans=$(ask_yes_no "Install CNV from redhat-operators catalog?" "$(bool_to_default_yn "$cnv_ui_install")")
            cnv_ui_install="false"
            if [[ "$cnv_ui_ans" == "y" ]]; then cnv_ui_install="true"; fi

            if [[ "$cnv_ui_install" == "false" ]]; then
                konflux_cnv_bundle=$(ask "Konflux CNV bundle image" "$konflux_cnv_bundle")
                custom_cnv_bundle=$(ask "Custom CNV bundle image" "$custom_cnv_bundle")
                fbc_stage_cnv_image=$(ask "CNV FBC image" "$fbc_stage_cnv_image")
            fi
        fi
    fi
    ...
    add_export CYPRESS_SKIP_CNV_INSTALL "$skip_cnv_install"
    add_export CYPRESS_CNV_UI_INSTALL "$cnv_ui_install"
    add_export_if_set CYPRESS_KONFLUX_CNV_BUNDLE_IMAGE "$konflux_cnv_bundle"
    add_export_if_set CYPRESS_CUSTOM_CNV_BUNDLE_IMAGE "$custom_cnv_bundle"
    add_export_if_set CYPRESS_FBC_STAGE_CNV_IMAGE "$fbc_stage_cnv_image"

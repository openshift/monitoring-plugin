...
const getRequiredConfigStatus = (
  ...
  const [uiPlugins] = requirementResources[RequirementGroupVersionKind.UIPlugin];
...
const isCapabilityLoaded = (
  ...
  const requirementKinds = new Set<RequirementGroupVersionKind>();

  if (!isEmpty(definition.requiredOperators)) {
    requirementKinds.add(RequirementGroupVersionKind.ClusterServiceVersion);
  }
  ...
  definition.requiredConfigs.forEach((config) => {
    if (
      ...
      requirementKinds.add(RequirementGroupVersionKind.UIPlugin);
    }
    if (config.groupVersionKind) {
...
export const getObservabilityCapability = (
  ...
  const [csvs] = requirementResources[RequirementGroupVersionKind.ClusterServiceVersion];

...
const buildRequirementResources = (
  ...
  Object.values(RequirementGroupVersionKind).forEach((kind) => {
...
const monitoringStackGVK =
  RequirementGroupVersionKinds[RequirementGroupVersionKind.MonitoringStack];
...
const buildRequiredOperator = (overrides: Partial<RequiredOperator> = {}): RequiredOperator => ({
  id: COO_ID,
  title: 'Cluster Observability Operator',
  groupVersionKind: RequirementGroupVersionKinds[RequirementGroupVersionKind.ClusterServiceVersion],
  keywords: 'cluster observability operator',
    ...
    requiredConfigs: [
      {
        ...
        title: 'COO MonitoringStack CR',
        type: RequiredConfigType.CustomResource,
        groupVersionKind: RequirementGroupVersionKind.MonitoringStack,
        requiredOperatorId: COO_ID,
      },
    ],
  };
  ...
  it('reports Ready when every operator and config is satisfied', () => {
    const capability = getObservabilityCapability(
      ...
      buildRequirementResources({
        [RequirementGroupVersionKind.ClusterServiceVersion]: [[cooCSV()], true, undefined],
        [RequirementGroupVersionKind.MonitoringStack]: [[monitoringStack], true, undefined],
      }),
    );
        ...
        requiredConfigs: [
          {
            ...
            title: 'LokiStack CR',
            type: RequiredConfigType.CustomResource,
            groupVersionKind: RequirementGroupVersionKind.LokiStack,
  ...
  describe('UIPlugin configs', () => {
    ...
    it('resolves by plugin type when only pluginName is given', () => {
      const definition = withPluginConfig({
        ...
        groupVersionKind: RequirementGroupVersionKind.UIPlugin,
      ...
      expect(
        getObservabilityCapability(
          definition,
          buildRequirementResources({
            [RequirementGroupVersionKind.UIPlugin]: [[monitoringPlugin(false)], true, undefined],
          }),
  ...
  describe('MonitoringFeature configs', () => {
    ...
    it('reports Missing when the monitoring UIPlugin is not installed', () => {
      ...
      expect(
        getObservabilityCapability(
          definition,
          buildRequirementResources({
            [RequirementGroupVersionKind.UIPlugin]: [[], true, undefined],
          }),
        ).requiredConfigs[0].status,
      ).toEqual(RequirementStatus.Missing);
    });
    ...
    it('reports Missing while UIPlugin resources are still loading', () => {
      ...
      const capability = getObservabilityCapability(
        definition,
        buildRequirementResources({
          [RequirementGroupVersionKind.UIPlugin]: [undefined, false, undefined],
        }),
      );
  ...
  describe('loaded', () => {
    it('is false while a kind the capability depends on is still loading', () => {
      const capability = getObservabilityCapability(
        monitoringDefinition,
        buildRequirementResources({
          [RequirementGroupVersionKind.MonitoringStack]: [undefined, false, undefined],
        }),
      );
    ...
    it('ignores kinds the capability does not depend on', () => {
      const capability = getObservabilityCapability(
        monitoringDefinition,
        buildRequirementResources({
          [RequirementGroupVersionKind.FlowCollector]: [undefined, false, undefined],
        }),
      );

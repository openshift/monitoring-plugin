...
func TestRBAC_PreviewAlertRule_Create(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			status, err := previewCreateWithToken(ctx, t, f, tc.token, tc.namespace, tc.alertName)
			if status != tc.wantStatus {
				t.Fatalf("Expected status %d, got %d: %v", tc.wantStatus, status, err)
			}
		})
	}
}
...
func TestRBAC_PreviewAlertRule_Update(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			status, err := previewUpdateWithToken(ctx, t, f, tc.token, tc.ruleID)
			if status != tc.wantStatus {
				t.Fatalf("Expected HTTP status %d, got %d: %v", tc.wantStatus, status, err)
			}
		})
	}
}

func previewCreateWithToken(ctx context.Context, t *testing.T, f *framework.Framework, token, namespace, alertName string) (int, error) {
	t.Helper()
...
func previewUpdateWithToken(ctx context.Context, t *testing.T, f *framework.Framework, token, ruleID string) (int, error) {
	t.Helper()

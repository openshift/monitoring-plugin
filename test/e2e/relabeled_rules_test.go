...
func listRules(ctx context.Context, f *framework.Framework) ([]monitoringv1.Rule, error) {
...
func TestRBAC_GetRules(t *testing.T) {
	...
	_, err = createPrometheusRule(ctx, f, nsY, testRule)
	if err != nil {
		t.Fatalf("Failed to create PrometheusRule: %v", err)
	}

	err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 2*time.Minute, true, func(ctx context.Context) (bool, error) {
		rules, status, err := listRulesWithToken(ctx, f, f.BearerToken, nsY)
		if err != nil {
			t.Logf("Admin GET /rules failed: %v", err)
			return false, nil
		}
		if status != http.StatusOK {
			t.Logf("Admin GET /rules returned status %d, retrying", status)
			return false, nil
		}
		if containsRule(rules, alertName) {
			return true, nil
		}
		t.Logf("Waiting for rule %s (admin sees %d rules)", alertName, len(rules))
		return false, nil
	})
	if err != nil {
		t.Fatalf("Timeout waiting for admin to see rule: %v", err)
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := containsRule(rules, alertName)
			if got != tc.wantRule {
				t.Fatalf("Rule %s visibility: want %v, got %v (%d rules returned)", alertName, tc.wantRule, got, len(rules))
			}
		})
	}
}

func containsRule(rules []monitoringv1.Rule, alertName string) bool {
	for _, r := range rules {
...
func listRulesWithToken(ctx context.Context, f *framework.Framework, token, namespace string) ([]monitoringv1.Rule, int, error) {
	...
	return allRules, resp.StatusCode, nil
}

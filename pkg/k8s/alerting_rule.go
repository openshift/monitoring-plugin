...
func newAlertingRuleManager(ctx context.Context, clientset *osmv1client.Clientset, config *rest.Config) (*alertingRuleManager, error) {
...
func (arm *alertingRuleManager) Create(ctx context.Context, ar osmv1.AlertingRule) (*osmv1.AlertingRule, error) {
	...
	cs, err := arm.clientsetForCtx(ctx)
	if err != nil {
		return nil, err
	}

	created, err := cs.MonitoringV1().AlertingRules(ClusterMonitoringNamespace).Create(ctx, &ar, metav1.CreateOptions{})
	if err != nil {
...
func (arm *alertingRuleManager) Update(ctx context.Context, ar osmv1.AlertingRule) error {
	...
	_, err = cs.MonitoringV1().AlertingRules(ClusterMonitoringNamespace).Update(ctx, &ar, metav1.UpdateOptions{})
	if err != nil {
...
func (arm *alertingRuleManager) Delete(ctx context.Context, name string) error {
	...
	err = cs.MonitoringV1().AlertingRules(ClusterMonitoringNamespace).Delete(ctx, name, metav1.DeleteOptions{})
	if err != nil {
		if errors.IsNotFound(err) {
			return nil
		}
...
func (arm *alertingRuleManager) clientsetForCtx(ctx context.Context) (*osmv1client.Clientset, error) {
	token := BearerTokenFromContext(ctx)
	if token == "" {
		return arm.clientset, nil
	}
	cs, err := newUserScopedClientsets(arm.config, token)
	if err != nil {

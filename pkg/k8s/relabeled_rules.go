...
func (rrm *relabeledRulesManager) collectAlerts(ctx context.Context, relabelConfigs []*relabel.Config) map[string]monitoringv1.Rule {
	...
	// Fetch all ARCs once from the informer cache (O(1) per-rule lookup below).
	// This avoids O(n) live API server calls inside the per-rule loop that would
	// cause exponential rate-limit backoff and stale cache data for new rules.
	arcByName := rrm.arcsByName(ctx)
...
// arcsByName builds a namespace/name → ARC map from the informer cache.
// Called once per sync cycle so that determineManagedBy can do O(1) lookups
// instead of one live API call per rule.
func (rrm *relabeledRulesManager) arcsByName(ctx context.Context) map[string]*osmv1.AlertRelabelConfig {
	if rrm.alertRelabelConfigs == nil {
		return nil
	}
	arcs, err := rrm.alertRelabelConfigs.List(ctx, "")
	if err != nil {
		log.Errorf("arcsByName: failed to list ARCs from cache: %v", err)
		return nil
	}
	m := make(map[string]*osmv1.AlertRelabelConfig, len(arcs))
	for i := range arcs {
		key := arcs[i].Namespace + "/" + arcs[i].Name
		m[key] = &arcs[i]
	}
	return m
}
...
func (rrm *relabeledRulesManager) determineManagedBy(promRule *monitoringv1.PrometheusRule, alertRuleId string, arcByName map[string]*osmv1.AlertRelabelConfig) (string, string) {
	...
	// Determine relabelConfigManagedBy only for platform rules using the
	// pre-fetched cache map; no live API call is made here.
	isPlatform := rrm.namespaceManager.IsClusterMonitoringNamespace(promRule.Namespace)
	var relabelConfigManagedBy string
	if isPlatform && arcByName != nil {
		arcName := GetAlertRelabelConfigName(promRule.Name, alertRuleId)
		key := promRule.Namespace + "/" + arcName
		if arc, found := arcByName[key]; found {
...
// DetermineManagedBy determines the managed-by labels for a single PrometheusRule
// alert rule. Callers that have a user-scoped context (e.g. tests) can pass a
// live AlertRelabelConfigInterface; a targeted Get is performed for that one rule.
func DetermineManagedBy(ctx context.Context, alertRelabelConfigs AlertRelabelConfigInterface, namespaceManager NamespaceInterface, promRule *monitoringv1.PrometheusRule, alertRuleId string) (string, string) {
	// Single-rule path: fetch only the specific ARC with RBAC enforcement on the
	// caller's context, then build a one-entry map for determineManagedBy.
	var arcByName map[string]*osmv1.AlertRelabelConfig
	if alertRelabelConfigs != nil && namespaceManager.IsClusterMonitoringNamespace(promRule.Namespace) {
		arcName := GetAlertRelabelConfigName(promRule.Name, alertRuleId)
		arc, found, err := alertRelabelConfigs.Get(ctx, promRule.Namespace, arcName)
		if err == nil && found {
			arcByName = map[string]*osmv1.AlertRelabelConfig{
				promRule.Namespace + "/" + arcName: arc,
			}
		}
	}
	rrm := &relabeledRulesManager{
	...
	return rrm.determineManagedBy(promRule, alertRuleId, arcByName)
}

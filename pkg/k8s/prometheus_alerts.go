...
func (pa *prometheusAlerts) getThanosTenancyResponse(ctx context.Context, path string, namespace string) ([]byte, error) {
	...
	baseURL := fmt.Sprintf("https://%s.%s.svc:%d", ThanosQuerierServiceName, ClusterMonitoringNamespace, port)
...
func (pa *prometheusAlerts) lookupThanosTenancyRulesPort(ctx context.Context) int32 {
	...
	service, err := pa.coreClient.Services(ClusterMonitoringNamespace).Get(ctx, ThanosQuerierServiceName, metav1.GetOptions{})
	if err != nil {

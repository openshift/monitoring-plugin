...
func newAlertRelabelConfigManager(ctx context.Context, clientset *osmv1client.Clientset, config *rest.Config) (*alertRelabelConfigManager, error) {
...
func (arcm *alertRelabelConfigManager) Create(ctx context.Context, arc osmv1.AlertRelabelConfig) (*osmv1.AlertRelabelConfig, error) {
	cs, err := arcm.clientsetForCtx(ctx)
	if err != nil {
		return nil, err
	}
	created, err := cs.MonitoringV1().AlertRelabelConfigs(arc.Namespace).Create(ctx, &arc, metav1.CreateOptions{})
	if err != nil {
...
func (arcm *alertRelabelConfigManager) Update(ctx context.Context, arc osmv1.AlertRelabelConfig) error {
	...
	_, err = cs.MonitoringV1().AlertRelabelConfigs(arc.Namespace).Update(ctx, &arc, metav1.UpdateOptions{})
	if err != nil {
...
func (arcm *alertRelabelConfigManager) Delete(ctx context.Context, namespace string, name string) error {
	...
	err = cs.MonitoringV1().AlertRelabelConfigs(namespace).Delete(ctx, name, metav1.DeleteOptions{})
	if err != nil {
		return fmt.Errorf("failed to delete AlertRelabelConfig %s/%s: %w", namespace, name, err)
	}
...
func (arcm *alertRelabelConfigManager) clientsetForCtx(ctx context.Context) (*osmv1client.Clientset, error) {
	token := BearerTokenFromContext(ctx)
	if token == "" {
		return arcm.clientset, nil
	}
	cs, err := newUserScopedClientsets(arcm.config, token)
	if err != nil {

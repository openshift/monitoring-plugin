...
func TestUpdatePlatformAlertRule_IgnoresProtectedLabels(t *testing.T) {
	...
	if createdARC != nil {
		for _, cfg := range createdARC.Spec.Configs {
			if string(cfg.TargetLabel) == "openshift_io_alert_rule_id" && cfg.Replacement == "fake" {
				t.Error("protected label openshift_io_alert_rule_id must not be overridden by the request")
			}
		}
	}
}

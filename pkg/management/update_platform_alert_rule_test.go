...
func TestUpdatePlatformAlertRule_UserRuleReturnsError(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "ENABLE_USER_WORKLOAD_ARCS") {
...
func TestDropPlatformAlertRule_CreatesARCWithIdStampAndDrop(t *testing.T) {
	...
	err := client.DropPlatformAlertRule(context.Background(), drPlatformRuleId)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result == nil {
...
func TestDropPlatformAlertRule_Idempotent(t *testing.T) {
	client, mockK8s := newDropRestoreClient(t)
	...
	if err := client.DropPlatformAlertRule(context.Background(), drPlatformRuleId); err != nil {
...
func TestRestorePlatformAlertRule_DeletesARCWhenOnlyDropRemains(t *testing.T) {
	client, mockK8s := newDropRestoreClient(t)
	deleted := false
	...
	if err := client.RestorePlatformAlertRule(context.Background(), drPlatformRuleId); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !deleted {
...
func TestRestorePlatformAlertRule_KeepsOtherConfigsRemovesDropOnly(t *testing.T) {
	client, mockK8s := newDropRestoreClient(t)
	deleted := false
...
func TestRestorePlatformAlertRule_DeletesARCWhenOnlyStampAndDropRemain(t *testing.T) {
	client, mockK8s := newDropRestoreClient(t)
	deleted := false
	var updated *osmv1.AlertRelabelConfig

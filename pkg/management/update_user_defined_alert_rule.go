...
// UpdateUserDefinedAlertRule updates an existing user-defined alert rule by its
// ID. It rejects platform-managed, operator-managed, and GitOps-managed rules,
// validates severity, rejects duplicate rule definitions, and returns the
// recomputed rule ID.
func (c *client) UpdateUserDefinedAlertRule(ctx context.Context, alertRuleId string, alertRule monitoringv1.Rule) (string, error) {
	...
	if err := c.migrateClassificationOverrideIfRuleIDChanged(ctx, namespace, name, alertRuleId, computedId); err != nil {
		return "", err
	}
...
// rules because the platform does not support ARCs in the user-workload
// namespace. When CMO adds that support, this function should migrate any
// existing ARC to the new rule ID.

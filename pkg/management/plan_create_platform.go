...
func (c *client) planCreatePlatformAlertRule(ctx context.Context, alertRule monitoringv1.Rule) (*createPlatformPlan, error) {
	...
	osmRule := toOSMRule(ruleWithStampedID(alertRule, computedRuleID))
...
func ruleWithStampedID(alertRule monitoringv1.Rule, ruleID string) monitoringv1.Rule {
	prepared := alertRule
	prepared.Labels = copyStringMap(alertRule.Labels)
	prepared.Labels[k8s.AlertRuleLabelId] = ruleID
	return prepared
}

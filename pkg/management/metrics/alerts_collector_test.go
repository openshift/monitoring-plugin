...
func (m *mockAlertsFetcher) EnrichAlerts(_ context.Context, _ k8s.GetAlertsRequest) ([]k8s.PrometheusAlert, []string, error) {
...
func TestAlertsCollector_FollowerExposesNoMetrics(t *testing.T) {
	mock := &mockAlertsFetcher{
		alerts: []k8s.PrometheusAlert{
			{Labels: map[string]string{"alertname": "HighCPU"}, State: "firing", ActiveAt: time.Now()},
		},
	}
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	collector := metrics.NewAlertsCollector(ctx, mock, 1*time.Hour, func() bool { return false })
	families := collectMetrics(t, collector)
	family := findFamily(families, metrics.MetricName)
	if family != nil && len(family.GetMetric()) != 0 {
		t.Errorf("follower should expose no metrics, got %d", len(family.GetMetric()))
	}
}

func TestNewEmptyHandler(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/metrics", nil)
	rec := httptest.NewRecorder()
	metrics.NewEmptyHandler().ServeHTTP(rec, req)
	...
	if strings.Contains(rec.Body.String(), metrics.MetricName) {
		t.Fatalf("empty handler must not expose %s, got %q", metrics.MetricName, rec.Body.String())
	}
}

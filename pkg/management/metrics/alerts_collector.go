...
// alerts via the management client's EnrichAlerts (which applies ARC relabeling
...
func NewHandler(ctx context.Context, fetcher AlertsFetcher, kubeConfig *rest.Config) (http.Handler, error) {
	...
	return handlerFor(registry), nil
}

// NewEmptyHandler returns a Prometheus /metrics handler with no series.
// Used when alert-management-api is disabled so platform scrapes still
// succeed (up=1) instead of falling through to the static file server.
func NewEmptyHandler() http.Handler {
	return handlerFor(prometheus.NewRegistry())
}
...
func (c *AlertsCollector) sync(ctx context.Context) {
	...
	alerts, _, err := c.fetcher.EnrichAlerts(ctx, k8s.GetAlertsRequest{})
	if err != nil {

...
1. **Set up test data first**:
   - Use the CSV file from [`simulate_scenarios/complete-test-data.csv`](../simulate_scenarios/complete-test-data.csv) with the simulation script from the `cluster-health-analyzer` repository (`local/simulate.sh`).
   - Alternatively, copy the CSV from the "Complete Test Data" section below.

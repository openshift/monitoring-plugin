...
const INCIDENT_IDS = Array.from(
  { length: INCIDENT_COUNT },
  (_, i) => `bench-${String(i + 1).padStart(2, "0")}`,
);
...
describe(
  "Performance: Endurance - Select/Deselect Cycling",
  ...
  () => {
    ...
    it("8.1 Endurance: Incident select/deselect cycling across 20 incidents", () => {
      cy.mockIncidentFixture(
        "incidents/scenarios/22-benchmark-20-incidents.yaml",
      );
      ...
      const runCycle = (cycleIndex: number) => {
        ...
        cy.window({ log: false }).then((win) => {
          ...
          if ((cycleIndex + 1) % 20 === 0 || cycleIndex === 0) {
            const totalMeasure = win.performance.measure(
              "endurance-progress",
              "endurance-start",
            );
